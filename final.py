...
if __name__ == "__main__":
    # Command-line arguments: c42copy.py function_name trace_file
    function_name = sys.argv[1]
    trace_file = sys.argv[2]

    # Call the specified function with the trace file
    globals()[function_name](trace_file)
...
class MemorySystem:
    def __init__(self, assoc):
        ...
        self.l2 = Cache(256, 64, assoc, self)
        self.dram = {}
...
# modified by adding associativity as param to simulate cache
def simulate_cache(trace_file, assoc):
    memory_system = MemorySystem(assoc)
    ...
    print()
    print("******* TIME *******")
    print("Time spent in L1 Data Cache:", (memory_system.active_time['l1_data']), "ns")
    print("Time spent in L1 Instruction Cache:", (memory_system.active_time['l1_inst']), "ns")
    print("Total time spent in L1 cache:", (memory_system.active_time['l1_inst'] + memory_system.active_time['l1_data']), "ns")
    print("Time spent in L2 Cache:", (memory_system.active_time['l2']), "ns")
    print("Time spent in DRAM:", (memory_system.active_time['dram']), "ns")
    ...
    print("******* DYNAMIC ENERGY *******")
    print("Energy spent in L1 Data Cache:", l1_inst_dynamic,"nJ")
    print("Energy spent in L1 Instruction Cache:", l1_data_dynamic,"nJ")
    print("Total energy spent in L1 Cache:", l1_data_dynamic + l1_inst_dynamic,"nJ")
    print("Energy spent in L2 Cache:", l2_dynamic,"nJ")
    print("Energy spent in DRAM:", dram_dynamic,"nJ")
    print("Total:", l1_data_dynamic + l1_inst_dynamic + l2_dynamic + dram_dynamic,"nJ")
   
simulate_cache('047.tomcatv.din')'''
    l1_inst_dynamic = memory_system.active_time['l1_inst'] * 1
    l1_inst_static = (memory_system.idle_time['l1_inst']) * 0.5 # in nJ
    l1_data_dynamic = memory_system.active_time['l1_data'] * 1
    l1_data_static = (memory_system.idle_time['l1_data']) * 0.5 # in nJ
    l2_dynamic = memory_system.active_time['l2'] * 2 + memory_system.penalties['l2']
    dram_dynamic = memory_system.active_time['dram'] * 4 + memory_system.penalties['dram']

    l1_inst_hits = memory_system.hits['l1_inst']
    l1_inst_misses = memory_system.misses['l1_inst']

    l1_data_hits = memory_system.hits['l1_data']
    l1_data_misses = memory_system.misses['l1_data']
    dram_accesses = memory_system.hits['dram']

    total_l1_hits = memory_system.hits['l1_data'] + memory_system.hits['l1_inst']
    total_l1_misses = memory_system.misses['l1_data'] + memory_system.misses['l1_inst']
    total_l2_hits = memory_system.hits['l2']
    total_l2_misses = memory_system.misses['l2']

    total_l1_static = (((memory_system.idle_time['l1_inst'] + memory_system.idle_time['l1_data']))* 0.5) # in nj
    total_l1_dynamic = l1_data_dynamic + l1_inst_dynamic
    
    total_l2_static = (memory_system.idle_time['l2'] * 0.8) # nj
    total_l2_dynamic = l2_dynamic

    total_dram_static = (memory_system.idle_time['dram'] * 0.8) # nJ
    total_dram_dynamic = dram_dynamic
    total_dram_energy =  total_dram_static + total_dram_dynamic #  nJ

    total_l1_energy = (total_l1_static + total_l1_dynamic)/(10**6)
    total_l2_energy = (total_l2_static + total_l2_dynamic)/(10**6)
    total_l1_inst_energy = (l1_inst_dynamic + l1_inst_static)/(10**6) # converts to mJ
    total_l1_data_energy = (l1_data_dynamic + l1_data_static)/(10**6) # converts to mJ

    total_dram_energy_final = (total_dram_energy)/(10**6)
    total_energy = (total_l1_static + total_l1_dynamic + total_l2_static + total_l2_dynamic + total_dram_energy)/(10**6)
    # total_time = (memory_system.active_time['dram'])/(10**6)
    total_time = (memory_system.global_clock)/(10**6)

    print("L1 inst Cache Hits: ", l1_inst_hits)
    print("L1 inst Cache Misses: ", l1_inst_misses)
    print("L1 inst Energy: ", total_l1_inst_energy)

    print("L1 data Cache Hits: ", l1_data_hits)
    print("L1 data Cache Misses: ", l1_data_misses)
    print("L1 data Energy: ", total_l1_data_energy)

    print("DRAM Accesses: ", dram_accesses)

    print("L1 Cache Hits: ", total_l1_hits)
    print("L1 Cache Misses: ", total_l1_misses)
    print("L2 Cache Hits: ", total_l2_hits)
    print("L2 Cache Misses: ", total_l2_misses)
    # print()

    print("L1 Energy(mJ): ", total_l1_energy) # in mJ
    print("L2 Energy(mJ): ", total_l2_energy)
  
    print("DRAM Energy(mJ): ", total_dram_energy_final)
    print("Total Energy(mJ): ", total_energy)
